...
def getRTTList(n1):
    ...
    n1.restart()
    n2 = n1.client('127.0.0.2', data=[])
    n1.restart()
    n3 = n1.client('127.0.0.3', data=[])
    n1.restart()
    n4 = n1.client('127.0.0.4', data=[])
    n1.restart()
    return n2, n3, n4
...
if __name__ == "__main__":
    ...
    node1 = Node('127.0.0.1', PORT)
    node1.server()
    node1RT["rtt"].append("0")
    print("node1, node1: " + node1RT["rtt"][-1])
    node1.restart()
    node1RT["rtt"].append(node1.client('127.0.0.2'))
    print("node2, node1: " + node1RT["rtt"][-1])
    node1.restart()
    node1RT["rtt"].append(node1.client('127.0.0.3'))
    print("node3, node1: " + node1RT["rtt"][-1])
    node1.restart()
    node1RT["rtt"].append(node1.client('127.0.0.4'))
